function [ ] = CalculateBRLDDictionary(opts, dictionary_opts)
...
if(dictionary_flag)
    ...
    image_dir=sprintf('%s/%s/',opts.localdatapathBRLD,num2string(1,8)); % location descriptor
    inFName = fullfile(image_dir, sprintf('%s', featureName));
    load(inFName, 'features');
    data = features.data;
    
    image_dir=sprintf('%s/%s/',opts.localdatapathBRLD,num2string(2,8)); % location descriptor
    inFName = fullfile(image_dir, sprintf('%s', 'BRLD_features'));
    load(inFName, 'features');
    ...
    for n=1:niters
        ...
        if n > 1
            ...
            if max(max(abs(centres - old_centres))) <0.01
                dictionary= centres;
