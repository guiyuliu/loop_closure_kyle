...
if(assign_flag)
    ...
    BOW=[];
    for ii=1:nimages
        image_dir=sprintf('%s/%s/',opts.localdatapathBRLD,num2string(ii,8));                    % location where detector is saved
   ...
   %% assign tf-idf weight
    ni=zeros(1,nimages);                                                                     % ni(i) is the total number of words in the ith image
    nw=zeros(1,vocabulary_size);                                                             % nw(j) is the number of images containing the jth word
    ...
    for p=1:vocabulary_size
        for q=1:nimages
            BOW(p,q)=BOW(p,q)/ni(q)*log(nimages/nw(p));
            if isnan(BOW(p,q))
                BOW(p,q)=0;
            end
        end
