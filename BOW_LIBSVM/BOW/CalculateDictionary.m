...
if(dictionary_flag)
    ...
    image_dir=sprintf('%s/%s/',opts.localdatapathSIFT,num2string(1,8)); % location descriptor
    inFName = fullfile(image_dir, sprintf('%s', featureName));
    load(inFName, 'features');
    data = features.data;
    
    image_dir=sprintf('%s/%s/',opts.localdatapathSIFT,num2string(2,8)); % location descriptor
    ...
    for n=1:niters
        ...
        for f = 1:nimages
            ...
            image_dir=sprintf('%s/%s/',opts.localdatapathSIFT,num2string(f,8)); % location descriptor
            inFName = fullfile(image_dir, sprintf('%s',featureName));
